...
# ✅ Validate VPC was created successfully
if [ "$VPC_ID" == "None" ] || [ -z "$VPC_ID" ]; then
    echo "ERROR: VPC creation failed. Aborting."
    exit 1
fi
echo "VPC ID: $VPC_ID"

# Enable DNS hostnames so that EC2 instances launched in this VPC
# receive a public DNS hostname
...
# ✅ Validate IGW was created successfully
if [ "$IGW_ID" == "None" ] || [ -z "$IGW_ID" ]; then
    echo "ERROR: Internet Gateway creation failed. Aborting."
    exit 1
fi
echo "IGW ID: $IGW_ID"
...
# ✅ Validate subnet was created successfully
if [ "$SUBNET_ID" == "None" ] || [ -z "$SUBNET_ID" ]; then
    echo "ERROR: Subnet creation failed. Aborting."
    exit 1
fi
...
# Description: Adds a default route pointing to the IGW so instances can
#              send and receive internet traffic over IPv4.
# =============================================================================
...
# ✅ Validate route table was found
if [ "$ROUTE_TABLE_ID" == "None" ] || [ -z "$ROUTE_TABLE_ID" ]; then
    echo "ERROR: Route Table not found. Aborting."
    exit 1
fi
